...
function TeamPanel({ side, team, match, setMatch, matchMode, onFixtureSelect }) {
  ...
  // Match mode: fixture selection
  const [fixtures, setFixtures] = useState(null) // null = not loaded
  ...
  async function loadFixtures() {
    ...
    try {
      const res = await fetch(`/api/fixtures?team=${encodeURIComponent(team.name)}`)
          ...
          >
            <div className="flex items-center justify-between mb-2">
              ...
              <button
                ...
                disabled={fixturesLoading || !team.name.trim()}
                className="text-xs px-3 py-1 rounded-lg text-white disabled:opacity-40 transition-colors"
                style={{ background: fixturesLoading ? '#374151' : '#15803d' }}
              >
                {fixturesLoading ? '⟳ Söker…' : fixtures === null ? 'Hämta matcher' : '↺ Uppdatera'}
              </button>
            </div>

            {fixturesError && (
              <p className="text-red-400 text-xs mb-2">{fixturesError}</p>
            )}

            {fixtures !== null && fixtures.length === 0 && (
              <p className="text-gray-500 text-xs">Inga matcher hittades för {team.name} i VM 2026.</p>
            )}
            ...
            {selectedFixture && (
              <p className="text-green-400 text-xs mt-2">
                ✓ Vald: {selectedFixture.home} vs {selectedFixture.away} · {formatFixtureDate(selectedFixture.date)}
              </p>
            )}
          </div>
        )}
